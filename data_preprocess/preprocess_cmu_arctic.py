...
def process_part(meta, output_dir, audio_dir):
    ...
    for row in tqdm(meta.iterrows(), total=len(meta)):
            ...
            caption = f'A {genders[gender]} reads out "{text}" in the {accents[acc]} accent'
...
def preprocess(dataset_name, num_process):
    ...
    data_dir = 'CMU_Arctic_extracted/'
    dfs = []

    for f in os.listdir(data_dir):
        p = data_dir+f+'/txt.done.data'
        df = pd.read_csv(p, sep='"', names=['filename', 'text', '_'])
        df['filename'] = df['filename'].apply(lambda x: f+'/'+x.replace('( ', '').replace(' ', '')+'.wav')
        df = df[['filename', 'text']]
        dfs.append(df)
...
if __name__=='__main__':
    preprocess(dataset_name='CMU_Arctic', num_process=4)
