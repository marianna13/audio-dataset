...
def process_part(meta, output_dir, audio_dir):
    ...
    for row in tqdm(meta.iterrows(), total=len(meta)):
        label, yt_id, start, end, label = row[1].values
        if 'generic' in label.lower():
            continue
        yt_id = yt_id.split('_')[0]
        if yt_id!=yt_id_prev:
        ...
        text = f'the sound of {label.lower()}'
        audio_json = {
                    'text': [text],
                    'tag': label.split(', '),
                    'original_data': {
...
def preprocess(dataset_name, num_process):
    ...
    meta_dir = '/home/ubuntu/marianna/clap/raw_dataset/audioset_strong/audioset_train_strong.tsv'

    meta_dir = '/home/ubuntu/marianna/clap/raw_dataset/audioset_strong/audioset_train_strong.tsv'
    meta_dir_eval = '/home/ubuntu/marianna/clap/raw_dataset/audioset_strong/audioset_eval_strong.tsv'
    labels_dir = '/home/ubuntu/marianna/clap/raw_dataset/audioset_strong/mid_to_display_name.tsv'

    meta = pd.read_csv(meta_dir, sep='\t')
    meta_eval = pd.read_csv(meta_dir_eval, sep='\t')
    meta = pd.concat([meta, meta_eval])
    labels = pd.read_csv(labels_dir, sep='\t', names=['label', 'label_name'])
    meta = meta.set_index('label').join(labels.set_index('label')).reset_index()
    meta = meta.sort_values(by=['segment_id'])
...
if __name__=='__main__':
    preprocess(dataset_name='audioset_strong', num_process=20)